...
func UnmarshalAttribute(an *apb.Any) (bgp.PathAttributeInterface, error) {
	...
	case *api.QualityAttribute:
		return bgp.NewPathAttributeQuality(a.Quality), nil
...
func NewQualityFromNative(a *bgp.PathAttributeQuality) (*api.QualityAttribute, error) {
	return &api.QualityAttribute{
		Quality: a.Value,
	}, nil
}
...
func MarshalPathAttributes(attrList []bgp.PathAttributeInterface) ([]*apb.Any, error) {
	...
	for _, attr := range attrList {
		...
		case *bgp.PathAttributeLocalPref:
			...
			n, _ := apb.New(v)
			anyList = append(anyList, n)
		case *bgp.PathAttributeQuality:
			v, err := NewQualityFromNative(a)
			if err != nil {
				return nil, err
			}
			n, _ := apb.New(v)
