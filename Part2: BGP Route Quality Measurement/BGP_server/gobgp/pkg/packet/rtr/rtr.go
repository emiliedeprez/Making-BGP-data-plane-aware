...
func (m *RTRIPPrefix) DecodeFromBytes(data []byte) error {
	...
	if m.Type == RTR_IPV4_PREFIX {
		...
		i := 20
		if i < int(m.Len) {
			m.MeasurementIP = []net.IP{}
			for i < int(m.Len) {
				m.MeasurementIP = append(m.MeasurementIP, net.IP(data[i:i+4]).To4())
				i += 4
			}
		}
	} else {
		...
		if i < int(m.Len) {
			m.MeasurementIP = []net.IP{}
			for i < int(m.Len) {
				m.MeasurementIP = append(m.MeasurementIP, net.IP(data[i:i+16]).To16())
				i += 16
			}
		}
	}
...
func (m *RTRIPPrefix) Serialize() ([]byte, error) {
	...
	binary.BigEndian.PutUint32(data[4:8], m.Len)
	lenght := m.PrefixLen
	if  m.MeasurementIP != nil {
		if  m.Type == RTR_IPV4_PREFIX {
			lenght += uint8(4 * len( m.MeasurementIP))
		} else {
			lenght += uint8(16 * len( m.MeasurementIP))
		}
	...
	if m.Type == RTR_IPV4_PREFIX {
		...
		if m.MeasurementIP != nil {
			i := 20
			for _, p := range m.MeasurementIP {
				copy(data[i:i+4], p.To4())
				i += 4
			}
		}
	} else {
...
func NewRTRIPPrefix(prefix net.IP, prefixLen, maxLen uint8, as uint32, flags uint8, measurementIP ...[]net.IP) *RTRIPPrefix {
	var MeasurementIP []net.IP
	var pduType uint8
	var pduLen uint32
	...
	if len(measurementIP) > 0 {
		MeasurementIP = measurementIP[0]
	}
