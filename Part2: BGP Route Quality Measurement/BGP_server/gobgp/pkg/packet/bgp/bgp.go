...
const (
	...
	BGP_ATTR_TYPE_QUALITY         BGPAttrType = 41
)
...
var PathAttrFlags map[BGPAttrType]BGPAttrFlag = map[BGPAttrType]BGPAttrFlag{
	...
	BGP_ATTR_TYPE_QUALITY:					BGP_ATTR_FLAG_OPTIONAL,
...
type PathAttributeQuality struct {
	PathAttribute
	Value uint32
}

func (p *PathAttributeQuality) DecodeFromBytes(data []byte, options ...*MarshallingOption) error {
	value, err := p.PathAttribute.DecodeFromBytes(data, options...)
	if err != nil {
		return err
	}
	if p.Length != 4 {
		eCode := uint8(BGP_ERROR_UPDATE_MESSAGE_ERROR)
		eSubCode := uint8(BGP_ERROR_SUB_ATTRIBUTE_LENGTH_ERROR)
		return NewMessageError(eCode, eSubCode, nil, "qualityf length isn't correct")
	}
	p.Value = binary.BigEndian.Uint32(value)
	return nil
}

func (p *PathAttributeQuality) Serialize(options ...*MarshallingOption) ([]byte, error) {
	var buf [4]byte
	binary.BigEndian.PutUint32(buf[:], p.Value)
	return p.PathAttribute.Serialize(buf[:], options...)
}

func (p *PathAttributeQuality) String() string {
	return "{Quality: " + strconv.FormatUint(uint64(p.Value), 10) + "}"
}

func (p *PathAttributeQuality) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		Type  BGPAttrType `json:"type"`
		Value uint32      `json:"value"`
	}{
...
func NewPathAttributeQuality(value uint32) *PathAttributeQuality {
	t := BGP_ATTR_TYPE_QUALITY
	return &PathAttributeQuality{
		PathAttribute: PathAttribute{
			Flags:  PathAttrFlags[t],
			Type:   t,
			Length: 4,
...
func GetPathAttribute(data []byte) (PathAttributeInterface, error) {
	...
	case BGP_ATTR_TYPE_QUALITY:
		return &PathAttributeQuality{}, nil
