...
func InitialConfig(ctx context.Context, bgpServer *server.BgpServer, newConfig *oc.BgpConfigSet, isGracefulRestart bool) (*oc.BgpConfigSet, error) {
	...
	for _, c := range newConfig.RQMServer {
		rules := make([]*api.RQMRule, 0, len(c.Rules))
		for _, r := range c.Rules {
			rules = append(rules, &api.RQMRule{
				TimeLimit: r.TimeLimit,
				SamplingInterval: r.SamplingInterval,
				Community: r.Community,
			})
		}
		if err := bgpServer.AddRQM(ctx, &api.AddRQMRequest{
			Address: c.Config.Address,
			Port:    c.Config.Port,
			Rules: rules,
			
		}); err != nil {
			bgpServer.Log().Fatal("failed to set rqm config",
				log.Fields{"Topic": "config", "Error": err})
		}
	}
