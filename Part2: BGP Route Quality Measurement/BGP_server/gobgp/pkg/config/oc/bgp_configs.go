...
type RQMConfig struct {
	Address string `mapstructure:"address" json:"address,omitempty"`
	Port uint32 `mapstructure:"port" json:"port,omitempty"`
}

func (lhs *RQMConfig) Equal(rhs *RQMConfig) bool {
	if lhs == nil || rhs == nil {
		return false
	}
...
type RQMRule struct {
	TimeLimit uint64 `mapstructure:"time-limit" json:"time-limit,omitempty"`
	SamplingInterval uint64 `mapstructure:"sampling-interval" json:"sampling-interval,omitempty"`
	Community string `mapstructure:"community" json:"community,omitempty"`
}

func (lhs *RQMRule) Equal(rhs *RQMRule) bool {
	if lhs == nil || rhs == nil {
		return false
	}
	if lhs.TimeLimit != rhs.TimeLimit {
		return false
	}
	if lhs.SamplingInterval != rhs.SamplingInterval {
		return false
	}
...
type RQMServer struct {
	Config RQMConfig `mapstructure:"config" json:"config,omitempty"`
	State  RQMState `mapstructure:"state" json:"state,omitempty"`
	Rules []RQMRule `mapstructure:"rule" json:"rule,omitempty"`
}

func (lhs *RQMServer) Equal(rhs *RQMServer) bool {
	if lhs == nil || rhs == nil {
		return false
	}
	if !lhs.Config.Equal(&(rhs.Config)) {
	...
	{
		lmap := make(map[string]*RQMRule)
		for i, l := range lhs.Rules {
			lmap[mapkey(i, string(l.Community))] = &lhs.Rules[i]
		}
		for i, r := range rhs.Rules {
			if l, y := lmap[mapkey(i, string(r.Community))]; !y {
				return false
			} else if !r.Equal(l) {
				return false
			}
...
// struct for container gobgp:state.
// State information relating to RQM server.
type RQMState struct {
	// original -> gobgp:up
	// gobgp:up's original type is boolean.
	Up bool `mapstructure:"up" json:"up,omitempty"`
	Request uint32 `mapstructure:"request" json:"request,omitempty"`
	Response uint32 `mapstructure:"response" json:"response,omitempty"`
	// This timer determines the amount of time since the
	// RQM last transitioned in of the Established state.
	Uptime int64 `mapstructure:"uptime" json:"uptime,omitempty"`
	// original -> gobgp:downtime
	// This timer determines the amount of time since the
	// RQM last transitioned out of the Established state.
	Downtime int64 `mapstructure:"downtime" json:"downtime,omitempty"`
	// original -> gobgp:last-pdu-recv-time
	// last time the received an pdu message from RQM server.
	LastPduRecvTime int64 `mapstructure:"last-pdu-recv-time" json:"last-pdu-recv-time,omitempty"`
	// original -> gobgp:rqm-messages
	// Counters for transmission and reception RQM Message types.
	RQMMessages RQMMessages `mapstructure:"rqm-messages" json:"rqm-messages,omitempty"`
}

// struct for container gobgp:rqm-messages.
// Counters for transmission and reception RQM Message types.
type RQMMessages struct {
	// original -> gobgp:rqm-sent
	// Counters for transmission RQM Message types.
	RQMSent RQMSent `mapstructure:"rqm-sent" json:"rqm-sent,omitempty"`
	// original -> gobgp:rqm-received
	// Counters for reception RQM Message types.
	RQMReceived RQMReceived `mapstructure:"rqm-received" json:"rqm-received,omitempty"`
}

// struct for container gobgp:rqm-received.
// Counters for reception RQM Message types.
type RQMReceived struct {
	// original -> gobgp:rqm-request
	// Number of request message received from RQM server.
	Request int64 `mapstructure:"rqm-request" json:"rqm-reqyest,omitempty"`
	// original -> gobgp:rqm-receive
	// Number of receive message received from RQM server.
	Reveived int64 `mapstructure:"rqm-receive" json:"rqm-receive,omitempty"`
	// original -> gobgp:error
	// Number of error message received from RQM server.
	Error int64 `mapstructure:"error" json:"error,omitempty"`
}

func (lhs *RQMReceived) Equal(rhs *RQMReceived) bool {
	if lhs == nil || rhs == nil {
		return false
	}
...
// struct for container gobgp:rqm-sent.
// Counters for transmission RQM Message types.
type RQMSent struct {
		// original -> gobgp:rqm-request
	// Number of request message received from RQM server.
...
func (lhs *RQMSent) Equal(rhs *RQMSent) bool {
	if lhs == nil || rhs == nil {
		return false
	}
	if lhs.Request != rhs.Request {
...
func (lhs *RQMMessages) Equal(rhs *RQMMessages) bool {
	if lhs == nil || rhs == nil {
		return false
	}
	if !lhs.RQMSent.Equal(&(rhs.RQMSent)) {
		return false
	}
	if !lhs.RQMReceived.Equal(&(rhs.RQMReceived)) {
		return false
	}
	return true
}
