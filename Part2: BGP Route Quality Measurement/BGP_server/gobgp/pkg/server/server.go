...
func NewBgpServer(opt ...ServerOption) *BgpServer {
	...
	s := &BgpServer{
		...
		roaTable:     roaTable,
		logger:       logger,
		rqmManager:   newRQMManager(logger),
	}
...
func (s *BgpServer) handleRQMQualityReceive(response *rqmEvent) {
	peer, found := s.neighborMap[response.Neighbor]
	if found {
		rib := s.globalRib
		path := response.Path
		t := rib.Tables[path.GetRouteFamily()]
		dest := t.GetDestination(path.GetNlri())
		for _, p := range dest.GetAllKnownPathList() {
			if p.GetSource() == path.GetSource() {
				if response.Quality == 0 {
					p.RemoveQuality()
					s.propagateUpdate(peer, []*table.Path{p})
				} else {
					q, _:= p.GetQuality()
					if ! p.HasQuality() || (q != uint32(response.Quality)) {
						p.SetQuality(uint32(response.Quality))
						s.propagateUpdate(peer, []*table.Path{p})
					}
				}
			}
...
func (s *BgpServer) Serve() {
	...
	for {
		...
		cases[3] = reflect.SelectCase{
			Dir:  reflect.SelectRecv,
			Chan: reflect.ValueOf(s.rqmManager.rqmCh.Out()),
		}
		...
		case 3:
			ev := value.Interface().(*rqmEvent)
			s.handleRQMQualityReceive(ev)
		default:
...
func (s *BgpServer) propagateUpdate(peer *peer, pathList []*table.Path) {
	...
	for _, path := range pathList {
		...
		measurmentIPs := s.roaTable.GetMeasurementIPs(path)
		if !path.IsWithdraw && len(s.rqmManager.clientMap) > 0 && len(measurmentIPs) > 0 && peer != nil && len(path.GetCommunities()) > 0 && !path.HasQuality(){
			client := reflect.ValueOf(s.rqmManager.clientMap).MapKeys()[0].String()
			for _, c := range path.GetCommunities() {
				if _, y := s.rqmManager.clientMap[client].maprules[c]; y{
					path.SetQuality(math.MaxUint32)
					s.rqmManager.eventCh.In() <- &rqmEvent{
						EventType : rqmSubscribe,
						Src: client,
						Neighbor: peer.ID(),
						Prefix: path.GetNlri().String(),
						Path: path,
						MeasurementIPs: measurmentIPs,
						Community: c,
					}
					break
				}
		...
		if path.IsWithdraw && len(s.rqmManager.clientMap) > 0 && peer != nil {
			client := reflect.ValueOf(s.rqmManager.clientMap).MapKeys()[0].String()
			s.rqmManager.eventCh.In() <- &rqmEvent{
				EventType : rqmUnsubscribe,
				Src: client,
				Neighbor: peer.ID(),
				Prefix: path.GetNlri().String(),
			}
		...
		if dsts := rib.Update(path); len(dsts) > 0 {
			for _, dst := range dsts {
				if len(dst.KnownPathList) > 0 && dst.KnownPathList[0].HasQuality(){
					if q, r := dst.KnownPathList[0].GetQuality(); r==nil && q == math.MaxUint32{
						dst.KnownPathList[0].SetQuality(1)
					}
				}
			}
...
func (s *BgpServer) ListRQM(ctx context.Context, r *api.ListRQMRequest, fn func(*api.Rqm)) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	var l []*api.Rqm
	err := s.mgmtOperation(func() error {	
		rules, err := s.rqmManager.GetRules()
		if err == nil {
			l = rules
		}
		return err
...
func (s *BgpServer) AddRQM(ctx context.Context, r *api.AddRQMRequest) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	return s.mgmtOperation(func() error {
		return s.rqmManager.AddServer(r.Address, r.Port, r.Rules)
	}, false)
}

func (s *BgpServer) DeleteRQM(ctx context.Context, r *api.DeleteRQMRequest) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	return s.mgmtOperation(func() error {
		return s.rqmManager.DeleteServer(r.Address)
	}, false)
}

func (s *BgpServer) EnableRQM(ctx context.Context, r *api.EnableRQMRequest) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	// return s.mgmtOperation(func() error {
	// 	return s.rqmManager.Enable(r.Address)
	// }, false)
	return nil
}

func (s *BgpServer) DisableRQM(ctx context.Context, r *api.DisableRQMRequest) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	// return s.mgmtOperation(func() error {
	// 	return s.rqm.Disable(r.Address)
	// }, false)
	return nil
}

func (s *BgpServer) ResetRQM(ctx context.Context, r *api.ResetRQMRequest) error {
	if r == nil {
		return fmt.Errorf("nil request")
	}
	// return s.mgmtOperation(func() error {
