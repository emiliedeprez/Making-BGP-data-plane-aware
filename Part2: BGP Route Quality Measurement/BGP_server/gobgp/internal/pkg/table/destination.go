...
func (dst *Destination) sort() BestPathReason {
	...
	sort.SliceStable(dst.knownPathList, func(i, j int) bool {
		...
		if better == nil {
			better = compareByQuality(path1, path2)
			reason = BPR_QUALITY
		}
		if better == nil {
...
func compareByQuality(path1, path2 *Path) *Path {
	//	Selects a path with highest quality
	// 
	//	The quality is a ranking between two routes. The quality is a ranking 
	//  between them. Lower the value is, higher the quality is.
	//
	quality1, err1 := path1.GetQuality()
	quality2, err2 := path2.GetQuality()
