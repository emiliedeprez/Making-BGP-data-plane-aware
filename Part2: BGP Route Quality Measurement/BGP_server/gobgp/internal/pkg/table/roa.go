...
type ROA struct {
	...
	AS            uint32
	Src           string
	MeasurementIP []net.IP
}

func NewROA(family int, prefixByte []byte, prefixLen uint8, maxLen uint8, as uint32, src string, MeasurementIP ...[]net.IP) *ROA {
	...
	var MeasurementIPcopy []net.IP
	if len(MeasurementIP) > 0 && MeasurementIP[0] != nil {
		for _, p := range MeasurementIP[0] {
			ip := make(net.IP, len(p))
			copy(ip, p)
			MeasurementIPcopy = append(MeasurementIPcopy, ip)
		}
	}
	return &ROA{
		Family: family,
		...
		AS:            as,
		Src:           src,
		MeasurementIP: MeasurementIPcopy,
	}
}
...
func (rt *ROATable) GetMeasurementIPs(path *Path) []string {
	if path.IsWithdraw || path.IsEOR() {
		// RPKI isn't enabled or invalid path
		return nil
	}
	...
	tree.WalkMatch(r, func(r *net.IPNet, v interface{}) bool {
		bucket, _ = v.(*roaBucket)
		for _, r := range bucket.entries {	
			if r.AS != 0 && r.AS == as && r.Network.String() == path.GetNlri().String() {
				ip_measurement = append(ip_measurement, r.MeasurementIP[0].String())
			}
		}
		return true
	})

	if len(ip_measurement) != 0 {
		return ip_measurement
	}
