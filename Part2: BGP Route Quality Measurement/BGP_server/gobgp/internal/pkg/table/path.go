...
func (path *Path) RemoveQuality() {
	if path.getPathAttr(bgp.BGP_ATTR_TYPE_QUALITY) != nil {
		path.delPathAttr(bgp.BGP_ATTR_TYPE_QUALITY)
	}
}
...
func (path *Path) GetQuality() (uint32, error) {
	attr := path.getPathAttr(bgp.BGP_ATTR_TYPE_QUALITY)
	if attr == nil {
		return 0, fmt.Errorf("no quality attr")
	}
	return  attr.(*bgp.PathAttributeQuality).Value, nil
}

func (path *Path) SetQuality(value uint32) {
	path.setPathAttr(bgp.NewPathAttributeQuality(value))
}

func (path *Path) HasQuality() (bool) {
	return (path.getPathAttr(bgp.BGP_ATTR_TYPE_QUALITY) != nil)
}
