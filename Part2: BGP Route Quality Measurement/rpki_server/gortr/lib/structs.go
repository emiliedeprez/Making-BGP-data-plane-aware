...
func (pdu *PDUIPv4Prefix) Write(wr io.Writer) {
	...
	binary.Write(wr, binary.BigEndian, uint16(0))
	lenght := 20
	if pdu.MeasurementIP != nil {
		lenght += 4 * len(pdu.MeasurementIP)
	}
	binary.Write(wr, binary.BigEndian, uint32(lenght))
	...
	if pdu.MeasurementIP != nil {
		for _, p := range pdu.MeasurementIP {
			binary.Write(wr, binary.BigEndian, p.To4())
		}
	}
}
...
func (pdu *PDUIPv6Prefix) Write(wr io.Writer) {
	...
	lenght := 32
	if pdu.MeasurementIP != nil {
		lenght += 16 * len(pdu.MeasurementIP)
	}
	binary.Write(wr, binary.BigEndian, uint32(lenght))
	binary.Write(wr, binary.BigEndian, pdu.Flags)
	binary.Write(wr, binary.BigEndian, uint8(mask))
	...
	if pdu.MeasurementIP != nil {
		for _, p := range pdu.MeasurementIP {
			binary.Write(wr, binary.BigEndian, p.To16())
		}
	}
}
