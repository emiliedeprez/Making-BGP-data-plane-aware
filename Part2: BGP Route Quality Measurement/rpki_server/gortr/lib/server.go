...
func (r1 ROA) Copy() ROA {
	...
	var MeasurementIP_copy []net.IP;
	if r1.MeasurementIP != nil{
		MeasurementIP_copy = []net.IP{}
		for _, p := range r1.MeasurementIP{
			ip := make(net.IP, len(p))
			copy(ip, p)
			MeasurementIP_copy = append(MeasurementIP_copy, ip)
		}
	}
	return ROA{
		Prefix: newprefix,
		...
		MaxLen: r1.MaxLen,
		Flags:  r1.Flags,
		MeasurementIP: MeasurementIP_copy}
}
...
func (c *Client) SendROA(roa ROA) {
	if roa.Prefix.IP.To4() == nil && roa.Prefix.IP.To16() != nil {
		pdu := &PDUIPv6Prefix{
			...
			ASN:    roa.ASN,
			Prefix: roa.Prefix,
			MeasurementIP: roa.MeasurementIP,
		}
		c.SendPDU(pdu)
