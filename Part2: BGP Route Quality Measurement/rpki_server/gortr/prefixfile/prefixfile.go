...
type ROAJson struct {
	...
	MeasurementIP []string 	   `json:"measurementIP,omitempty"`
}
...
func (roa *ROAJson) GetMeasurementIP() ([]net.IP, error) {
	if roa.MeasurementIP == nil { return nil, nil}
	prefixes := []net.IP{};
	for _, p := range roa.MeasurementIP{
		prefix := net.ParseIP(p)
		if prefix == nil {
			return nil, errors.New(fmt.Sprintf("Could not decode prefix: %v as part of ROA", roa.MeasurementIP))
		}
