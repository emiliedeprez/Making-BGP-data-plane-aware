...
type BgpServer struct {
	...
	logger           log.Logger
	localPrefManager *localPrefManager
}
...
func NewBgpServer(opt ...ServerOption) *BgpServer {
	...
	s := &BgpServer{
		...
		roaTable:         roaTable,
		logger:           logger,
		localPrefManager: newLocalPrefManager(logger),
	}
...
type localPrefNotify struct {
	Neighbor string
	NLRI     *table.Path
}

func (s *BgpServer) handleLocalPrefReceive(response *localPrefNotify) {
...
func (s *BgpServer) Serve() {
	...
	for {
		...
		cases[3] = reflect.SelectCase{
			Dir:  reflect.SelectRecv,
			Chan: reflect.ValueOf(s.localPrefManager.localPrefCh.Out()),
		}
		...
		case 3:
			ev := value.Interface().(*localPrefNotify)
			s.handleLocalPrefReceive(ev)
		default:
...
func (s *BgpServer) propagateUpdate(peer *peer, pathList []*table.Path) {
	...
	for _, path := range pathList {
		...
		lo, _ := path.GetLocalPref()
		if path.IsWithdraw && s.bgpConfig.Global.Config.LocalPref0 != (oc.LocalPrefConfig{}) && lo != 0 && peer != nil && ! peer.isIBGPPeer() {
			comm, _ := table.ParseCommunity(s.bgpConfig.Global.Config.LocalPref0.LocalPref0Community)
			t := rib.Tables[path.GetRouteFamily()]
			dest := t.GetDestination(path.GetNlri())
			for _, p := range dest.GetAllKnownPathList() {
				if p.GetSource() == path.GetSource() && peer != nil {
					for _, c := range p.GetCommunities() {
						if comm == c {
							path = p.Clone(false)
							path.SetLocalPref(0)
							s.localPrefManager.eventCh.In() <- &LocalPrefEvent{
								Time:     s.bgpConfig.Global.Config.LocalPref0.LocalPref0Timer,
								Neighbor: peer.ID(),
								NLRI:     path,
							}
							break
...
func (s *BgpServer) propagateUpdateToNeighbors(source *peer, newPath *table.Path, dsts []*table.Update, needOld bool) {
	...
	for _, targetPeer := range s.neighborMap {
		...
		// check pour local-pref = 0
		if s.bgpConfig.Global.Config.LocalPref0 != (oc.LocalPrefConfig{}) && !targetPeer.isIBGPPeer() {
			for i, path := range bestList {
				if path != nil {
					if v, _ := path.GetLocalPref(); v == 0 && !path.IsWithdraw {
						bestList = append(bestList[:i], bestList[i+1:]...)
						if oldList != nil {
...
func (s *BgpServer) GetBgp(ctx context.Context, r *api.GetBgpRequest) (*api.GetBgpResponse, error) {
	...
	s.mgmtOperation(func() error {
		...
		localpref0 := &api.LocalPrefConfig{}
		if g.Config.LocalPref0 != (oc.LocalPrefConfig{}) {
			localpref0.LocalPref_0Community = g.Config.LocalPref0.LocalPref0Community
			localpref0.LocalPref_0Timer = g.Config.LocalPref0.LocalPref0Timer
		}
